...
function xoaKhoaHoc()
{
    ...
    let flag = false;
    for(let id of mangXoa){
        for(let kh of dskh.mangKhoaHoc){
            if(id == kh.MaKhoaHoc){
    ...
    const res = dskh.XoaKhoaHoc(mangXoa,dskh.mangKhoaHoc)
    updateData('dskh',res);
    swal({
        text: `Xoá thành công`,
        icon: "success",
...
$("#btnInsertCourse2").click(function(e){
    e.preventDefault()
    // ThemKhoaHoc();
});
