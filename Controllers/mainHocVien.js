...
dshv.mangHocVien = getData('dsnd')
let dskh = getData('dskh')
loadHocVien(dshv);
...
function xoaHocVien()
{
    ...
    let string = ""
    for(let kh of dskh){
        for(let dk of kh.UserDangKy){
            for(let xoa of mangXoa){
                if(dk == xoa){
                    let newString = string === "" ? string : string + ", "
                    string = newString + xoa
                }
            }
        }
    }
    ...
    if(string !== ""){
        return swal({
        text: `
                Các tài khoản này đã đăng ký khoá học: 
                ${string}
            `,
            icon: "error",
            button: "OK",
    ...
    const res = dshv.XoaHocVien(mangXoa);
    updateData('dsnd',res);
    swal({
        text: `Xoá thành công`,
        icon: "success",
...
function layThongTinHocVien(name) {
    ...
    if (list != null) {
        ...
        localStorage.setItem("updateNguoiDung",JSON.stringify(list))
    }
}
